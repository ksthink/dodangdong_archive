...
즉 **손님 읽기는 없다.** 코드와 주석에 남아 있는 "손님" 은 DB 의 `anon` 역할을 가리킨다.
RLS 의 `guest_read` 정책은 아직 살아 있어서, publishable 키로 Supabase 를 곧장 부르면 공개 행은
읽힌다(README 의 "아직 열려 있는 틈"). 앱 밖의 이 틈을 닫는 것은 따로 결정할 일이다.
