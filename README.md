...
- **인물의 얼굴**: 사진을 따로 올리지 않는다. 관리 화면에서 **그 사람이 나오거나 만든 자료의 썸네일** 가운데
  하나를 고르면 `person.face_file_id` 가 그것을 가리킨다. 얼굴에도 출처가 남고, 그 자료를 지우면 얼굴도 저절로
  떨어진다. 고르지 않으면 디더 면에 호칭 첫 글자가 온다.
