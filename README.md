...
- **닫힌 아카이브다.** 로그인하지 않으면 어떤 화면도 열리지 않고 인트로(`/intro`)로 돌아간다.
- 자료를 올리고 고치고 지우는 것은 **관리자 한 사람**만 한다. 가입 화면은 없다.
- 지금까지의 결정과 남은 일은 **[HANDOFF.md](HANDOFF.md)** 에 있다.
...
| 경로 | |
| --- | --- |
| `/intro` | 인트로. 낡은 단말기 화면을 흉내 낸 문지기다. [입장] 을 누르면 로그인 칸이 열린다 |

로그인하지 않은 채로 다른 주소에 닿으면 `/intro?next=<가려던 곳>` 으로 보내고, 들어온 뒤 그리로 데려다준다.
`/api/*` 는 화면이 아니므로 401 을 돌려준다. 부팅 연출은 그 브라우저의 첫 방문에만 돈다.
...
- **영상**: 손님에게는 **재생 규격** 파일만 튼다. 규격은 MP4 · 영상 H.264 · 음성 AAC · 목차 앞(웹 최적화)이다.
  - 규격에 맞는 영상은 올린 그대로 재생한다.
  - 규격 밖(아이폰 HEVC 따위)은 원본으로 보존하고, 관리 화면의 **재생용 올리기**로 규격 파일을 따로 붙인다(`role = 'stream'`).
    변환은 관리자 PC 에서 한다. HandBrake "Fast 1080p30" + Web Optimized, 아이폰은 카메라 포맷 "높은 호환성".
  - 올릴 때 브라우저가 mp4 머리를 읽어 길이·크기·코덱·목차 위치를 적는다(`src/lib/mp4.ts`).
  - `scripts/tidy-videos.mjs` 는 이 변환을 ffmpeg 로 대신하는 선택 도구다(긴 영상은 오래 걸린다).
- **인물의 얼굴**: 사진을 따로 올리지 않는다. 관리 화면에서 **그 사람이 나오거나 만든 자료의 사진**을 고르고
  그 위에서 얼굴 자리를 잘라 쓴다(옛 집안 사진은 거의 단체 사진이라 독사진이 드물다).
  - 사진을 **누르면 그 자리가 네모의 가운데**가 되고, 크기는 슬라이더로 바꾼다. 옆에 실제로 보일 모습이 뜬다.
  - 저장하면 브라우저가 **원본에서** 256×256 으로 잘라 `role = 'face'` 파생 파일로 올린다(`derived_from` = 원본).
    원본은 손대지 않고, 어느 사진에서 왔는지 남으며, 그 자료를 지우면 얼굴도 저절로 떨어진다.
  - 네모를 놓았던 자리는 `person.face_crop` 에 비율로 적어 둔다. 다시 고칠 때 그 자리에서 시작하고,
    그 자체가 "이 사진의 이 자리에 이 사람이 있다"는 기록이기도 하다.
  - 고르지 않으면 디더 면에 호칭 첫 글자가 온다.
...
- 밝은 화면 하나뿐이다. 다크 모드는 없다(`color-scheme: only light`).
...
- **`src/proxy.ts`**: Next 16 에서 middleware 를 대신한다. 인트로와 그 화면이 쓰는 파일(`/fonts/*`, `/brand/*`)을 뺀
  **모든 길**에 문을 단다. 세션이 없으면 `/intro` 로 보내고, `/admin/*` 은 거기에 더해 `admin_user` 인지 확인한다.
...
| `node scripts/tidy-videos.mjs` | (선택) 영상 원본에서 재생용 사본·썸네일을 ffmpeg 로 만들어 붙인다. 드라이런이 기본 |
